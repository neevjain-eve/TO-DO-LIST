...
exports.handler = async (event) => {
  ...
  if (event.httpMethod === 'POST') {
    if (me.role !== 'admin' && me.role !== 'manager') return json(403, { error: 'Only an admin or manager can assign tasks.' });
    const body = parseBody(event);
    const title = (body.title || '').trim();
    if (!title) return json(400, { error: 'Title is required.' });
    if (!body.dueDate) return json(400, { error: 'Due date is required.' });

    const employees = await readJSON('employees', []);

    if (me.role === 'manager' && body.department) {
      return json(403, { error: 'Whole-department assignment isn\'t available to managers -- select your direct reports individually.' });
    }
    ...
    if (me.role === 'manager') {
      ...
      if (outsideReports) return json(403, { error: 'You can only assign tasks to employees who report to you.' });
    }

    if (!targetIds.length) return json(400, { error: 'No valid employees to assign to.' });
  ...
  if (event.httpMethod === 'PATCH') {
    ...
    if (me.role === 'employee') {
      ...
      if (!mine) return json(403, { error: 'This task is not assigned to you.' });
      if (!['Pending', 'In Progress', 'Completed'].includes(body.status)) return json(400, { error: 'Invalid status.' });
      ...
      if (isFullReportsTask) {
        ...
        return json(403, { error: 'You can only edit tasks assigned entirely to your own direct reports.' });
      }
  ...
  if (event.httpMethod === 'DELETE') {
    if (me.role !== 'admin' && me.role !== 'manager') return json(403, { error: 'Only an admin or manager can delete tasks.' });
    if (me.role === 'manager') {
      const employees = await readJSON('employees', []);
      if (!allAssigneesReportTo(task, employees, me.managerId)) {
        return json(403, { error: 'You can only delete tasks assigned entirely to your own direct reports.' });
      }
    }
    if (graph.configured()) {
      const employees = await readJSON('employees', []);
