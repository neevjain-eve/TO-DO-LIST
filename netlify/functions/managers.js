...
exports.handler = async (event) => {
  ...
  if (me.role !== 'admin') return json(403, { error: 'Only an admin can manage managers.' });
  ...
  if (event.httpMethod === 'GET') {
    ...
    return json(200, { managers: managers.map((m) => ({ ...m, username: usernameOf(m.id) })) });
  }
  ...
  if (event.httpMethod === 'POST') {
    ...
    return json(201, { manager, username, tempPassword: password });
  }
  ...
  if (event.httpMethod === 'PATCH') {
    ...
    if (!mgr) return json(404, { error: 'Manager not found.' });
    ...
    return json(200, { manager: mgr, username: newUsername });
  }
